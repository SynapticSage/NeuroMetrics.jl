function out = DFA_spikingtrigger_v3(index,exclude,pos,linpos,spikes,cellinfo,varargin)
...
ip.addParameter('tuneonly2endwell',true,@islogical);
...
tuneonly2endwell = ip.Results.tuneonly2endwell;
...
for jj = 1:numel(trigger_time) % For each well/goal type
    for ii = 1:size(inclusion_period_well{jj},1) % For each time the animal visits that well, capture a window
        ...
        % Perform special procedure if only tuning to current well
        if tuneonly2endwell
        end
