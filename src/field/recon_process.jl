...
function perform_reconstructions_marginals_and_error(beh, spikes,
    ...
    what,under = [vec(x) for x in eachrow(cat(split.(recon_summary.model,"|")...; dims=2))]
    recon_summary.what, recon_summary.under = what, under
    recon_summary = sort(recon_summary, [:model, :area, :unit])
