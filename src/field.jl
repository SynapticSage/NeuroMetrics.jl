module field
    ...
    module model
        ...
        """
        function poisson(field, grid=nothing, return_binary_tensor=true)
            λ  = field;
            if grid == nothing
                func(k) = (exp(λ) * λ^k)/factorial(k)
            lse
                G(k) = grid_lookup_func(grid, return_binary_tensor=true)
                if return_binary_tensor
                    func(k) = (exp(λ) * λ^G(k))/factorial(G(k))
                else # single event :: i dunno probably wrong
                    func(k) = (exp(λ(G(k))) * λ^1)/factorial(1)
                end
            end
            return func
        end
        function grid_lookup_func(grid; return_binary_tensor=true)
        end
    end
    export model
...
Graveyard
        ...
        function show_all_field_comparisons(sets...; keysubset=nothing)
            if keysubset ≠ nothing
                if keysubset isa Function
                    sets = [filter(keysubset, set)
                            for set ∈ sets]
                else
                    sets = [filter(x -> x[1] ∈ keysubset, set)
                            for set ∈ sets]
                end
            end
            tmp = length(sets[1])
            println("Length of sets[1] = tmp")

            completion_keys = []
            for i in 1:length(sets)
                for name in keys(sets[i])
                    append!(completion_keys, name)
                end
            end
            completion_keys = unique(completion_keys)
            plot_sets = []
            heatmap_collections = Dict()
            for key in completion_keys
                println(key)
                field_sets = []
                for i ∈ 1:length(sets) 
                    if key ∈ keys(sets[i]) && sets[i][key] ≠ nothing
                        push!(field_sets, heatmap(sets[i][key], 
                                              xticks=[], yticks=[], colorbar=false, 
                                              padding=(0,0), ylims=(-Inf,Inf), 
                                              xlims=(-Inf,Inf), c=:vik, 
                                              showaxis=:no) 
                              )
                    else
                        push!(field_sets, Plots.plot(showaxis=:no, xticks=[], yticks=[]))
                    end
                end
                heatmap_collections[key] = Plots.plot(field_sets...; 
                                                      padding=(0,0,0,0), 
                                                      margin=-1.5mm, 
                                                      title=key, 
                                                      layout=grid(1, length(sets)), 
                                                      plot_titlefontsize=4)
            end

            return heatmap_collections
        end
        function show_all_fields(fields, cell=nothing; 
                textcolor=:black, location=:bottom)

            fields = filter(x->x[2]!=nothing && x!=Nothing, fields)
            fields = sort(fields; by=x->x[1])
            Φ_detect = copy(Dict(x[1] => quantile(skipnan(vec(x[2])),[0.95,0.98])
                                 for x ∈ fields))
            clims    = Dict(x[1] => quantile(skipnan(vec(x[2])),[0.05,0.99]) for x
                            ∈ fields)
            # Filter out inactivity
            fields = filter(x -> Φ_detect[x[1]] != [0,0], fields)
            clims  = filter(x -> Φ_detect[x[1]] != [0,0], clims)

            heatmaps = [heatmap(fields[name], clims=Tuple(clims[name]), 
                                xticks=[], yticks=[], colorbar=false, 
                                padding=(0,0), ylims=(-Inf,Inf), 
                                xlims=(-Inf,Inf), c=:vik, showaxis=:no)
                        for name ∈ keys(fields)]
            R(start, stop) = abs(stop-start);
            x_remap(start,stop) = (stop - R(start,stop)*0.9, stop);
            y_remap(start,stop) = (stop - R(start,stop)*0.9, stop);
            for (key, hm) ∈ zip(keys(fields), heatmaps)
                # Remap x and y
                x = xlims(hm);
                y = ylims(hm);
                new_x = x_remap(x...)
                ...
                xlims!(hm, new_x)
                ylims!(hm, new_y)
                # Now compute where to put the text
                y = ylims(hm);
                if location == :bottom
                    y = minimum(ylims(hm)) * 1.15;
                elseif location == :top
                    y = maximum(ylims(hm)) * 0.85;
                end
                x = maximum(xlims(hm)) * 0.99;
                if cell != nothing
                    unit = findfirst(cell.unit.==key)
                    color = textcolor
                    font = :bold
                    if unit isa Nothing
                        annotation = string(key) 
                        area = nothing
                    else unit isa DataFrame
                        area = cell[unit,"area"]
                        annotation = string(key) * "\n" * area
                        if area == "PFC"
                            annotation = string(key) * "\n" *  
                                                latexstring("\\textit{", area, "}")
                        end
                    end
                    annotate!(hm, x, y, text(annotation, location, color, :right, :bold, 4)) 
                end
            end
            heatmap_set = Plots.plot(heatmaps[1:end]...; padding=(0,0,0,0), margin=-1.5mm)
            return heatmap_set
        end
"""
