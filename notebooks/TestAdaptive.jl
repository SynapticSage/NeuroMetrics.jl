...
@bind unit PlutoUI.Slider(sort(unique(spikes.unit)), show_value=true)
...
df = unstack(Table.to_dataframe(X), :unnamed, :value)

# ╔═╡ f001cdd1-fb4b-4f40-bfcd-be9fea6c8e5b
heatmap(names(df), 1:size(df,1), Matrix(ismissing.(df)), title="Missing values\nblack = not missing, white = missing")
...
shifted = Timeshift.shifted_fields(beh, spikes, -2:0.2:2, [:x,:y], widths=3f0)
