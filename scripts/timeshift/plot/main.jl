...
@softscope for (i,(animal,day,brain_area,frac)) in collect(enumerate(datasets))
    ...
    try
       ...
       for (k1, k2) in comparisons
           ...
           if (k1,k2) == (:cue, :memory)
               shifts_local = DIutils.in_range(shifts, [-.21, .61])
       ...
       for (k1, k2) in comparisons
           ...
           sort_bs = sortperm(f1[:bestshift_bitsperspike][:,1])
           XX1 =bps  = vcat([DIutils.norm_extrema(f)[DIutils.na, :]
                        for f in 
           ...
           sort_bs = sortperm(f2[:bestshift_bitsperspike][:,1])
           XX2 =bps  = vcat([DIutils.norm_extrema(f)[DIutils.na, :]
                        for f in 
       ...
       for (k1, k2) in comparisons
           ...
           for d in D
               ...
               val = DIutils.norm_extrema(val,[0,1])
               push!(R,val)
               push!(L, argmax(val))
           end
            ind = sortperm(L)
       ...
       for (k1, k2) in comparisons
           ...
           for d in D
               ...
               b1, b2 = DIutils.norm_extrema(b1,[0,1]), DIutils.norm_extrema(b2,[0,1])
               val = DIutils.norm_extrema(b2-b1,[0,1])
               push!(R,val)
               push!(L, argmax(val))
           end
            ind = sortperm(L)
       ...
       for k in keys(keyz)
           ...
           inds = sortperm(f[:bestshift_bitsperspike][:,1])
           bps  = f[:bitsperspike][inds, :]
           XX = hcat([DIutils.norm_percent(b,0.5) for b in eachrow(bps)]...)'
           h=heatmap(shifts, collect(1:size(XX,1)), XX; label="",
                     title="$(keyz[k].datacut)", size=(600,1200), c=:hot, clim=(0.3,1))
            vline!([0],c=:pink,linestyle=:dash, linewidth=3,label="")
            Plot.save((;k))
